...
function ToolImages({ images }: { images: NonNullable<ToolCall["images"]> }) {
  ...
  const rendered = images.flatMap((image) => {
  ...
  const selected = active === null ? undefined : rendered[active];
  return (
    <>
      <div className="mt-2 grid grid-cols-2 gap-2">
        {rendered.map((image, index) => (
          <button key={`${image.src}-${index}`} onClick={() => setActive(index)} className="group/image relative overflow-hidden rounded-[4px] border border-line2 bg-void text-left" title={language === "zh-CN" ? "点击预览图片" : "Preview image"}>
            <img src={image.src} alt="Tool output" className="max-h-44 w-full object-contain transition-transform duration-200 group-hover/image:scale-[1.015]" />
      ...
      {selected && <div role="dialog" aria-modal="true" aria-label={language === "zh-CN" ? "图片预览" : "Image preview"} onClick={() => setActive(null)} className="fixed inset-0 z-[90] flex items-center justify-center bg-void/90 p-8 backdrop-blur-sm animate-fade-up">
        ...
        <img onClick={(event) => event.stopPropagation()} src={selected.src} alt="Tool output enlarged" className="max-h-full max-w-full rounded-[5px] border border-line2 bg-void object-contain shadow-2xl" />
      </div>}
    </>
  );
}
