...
**应用会话（App Session）**：
用户在 Grox 中创建、查看、重开和删除的对话身份；它拥有目录项、草稿、提示队列和应用会话 journal。
_Avoid_: 当前页面、当前聊天框

**应用会话 Journal（App Session Journal）**：
按应用会话持久保存的版本化展示记录与回合状态；它负责界面连续性，但不替代 Agent 会话的模型上下文历史。
_Avoid_: 临时缓存、Agent 历史副本、唯一真相

**Agent 会话（Agent Session）**：
Grok Build CLI 通过 ACP 创建或恢复的上下文身份；所有协议事件和操作都必须显式归属到它。v0.3.2 中它与应用会话暂用同一 ID，但语义与持久化职责不同。
