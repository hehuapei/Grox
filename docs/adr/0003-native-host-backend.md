...
| 4 | Background execution | Host 自动化时钟、租约、建会话、模型/模式绑定、Prompt 和结算已落地；普通前台回合继续下沉 |
...
| 自动化 | Host 已拥有时钟、建会话、能力租约、模型/模式、Prompt 与结算；页面仍承担会话登记屏障 | runner 复用 SessionManager | 继续把页面登记屏障改为 Host 快照/事件，最终复用统一会话服务 |
